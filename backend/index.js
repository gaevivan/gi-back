...
MongoClient.connect('mongodb+srv://ghub-1-dbuser:Meph1stopheles@cluster0.iczte.mongodb.net/test?retryWrites=true&w=majority', { useUnifiedTopology: true })
  .then(client => {
    ...
    app.post('/api/entity', api.checkToken, (request, response) => api.entity(request, response));
    app.post('/api/select', api.checkToken, (request, response) => api.select(request, response));
    app.post('/api/update', api.checkToken, (request, response) => api.update(request, response));
    app.post('/api/remove', api.checkToken, (request, response) => api.remove(request, response));
    app.post('/api/create', api.checkToken, (request, response) => api.create(request, response));
